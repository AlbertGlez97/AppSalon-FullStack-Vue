...
const getAdminAppointmentsPending = async (req, res) => {
  ...
  try {
    ...
    if (!appointments.length) {
      const error = new Error(
        "No se encontraron citas para este usuario administrador"
      );

      return res.status(404).json({ msg: error.message });
    }

    res.status(200).json(appointments); // Devolver las citas encontradas del usuario administrador
  } catch (error) {
    console.error(error);
    res
      .status(500)
      .json({ msg: "Error al obtener las citas del usuario administrador" });
  }
};
